...
public class Queries {
	...
	public void insertStore(Store entry) {
		...
		values.put("updated_at", entry.getUpdated_at());
		values.put("date_for_ordering", entry.getDate_for_ordering());
		values.put("buy_ticket", entry.getBuy_ticket());
	...
	public ArrayList<Store> getStores() {
		...
		if (!mCursor.isAfterLast()) {
			do {
				...
				try {
					entry.setDate_for_ordering(mCursor.getString( mCursor.getColumnIndex("date_for_ordering")) );
					entry.setBuy_ticket(mCursor.getString( mCursor.getColumnIndex("buy_ticket")) );
				} catch (Exception e) {
					e.printStackTrace();
				}
				list.add(entry);
...
//		Cursor mCursor = db.rawQuery("SELECT * FROM categories ORDER BY category ASC", null); 
		Cursor mCursor = db.rawQuery("SELECT * FROM categories ORDER BY category_id ASC", null);
		mCursor.moveToFirst();
